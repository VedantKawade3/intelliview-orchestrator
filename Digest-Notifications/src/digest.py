...
from database import get_db_conn, init_db, DB_FILE
    ...
    def do_GET(self):
        ...
        if path in ('/', '/index.html'):
            ...
            with FILE_LOCK:
                ...
                cursor.execute("SELECT * FROM interviews ORDER BY date ASC, time ASC LIMIT ? OFFSET ?", (limit, offset))
                rows = cursor.fetchall()
                conn.close()
            ...
            with FILE_LOCK:
                ...
                total = cursor.fetchone()[0]
                cursor.execute("SELECT * FROM sent_logs ORDER BY timestamp DESC LIMIT ? OFFSET ?", (limit, offset))
                rows = cursor.fetchall()
                conn.close()
    ...
    def do_POST(self):
        ...
        if path == '/api/interviews':
            ...
            try:
                ...
                self.send_json(400, {"status": "error", "message": "Invalid date format. Expected YYYY-MM-DD."})
                return

            try:
                datetime.datetime.strptime(data['time'], "%H:%M")
            except ValueError:
                self.send_json(400, {"status": "error", "message": "Invalid time format. Expected HH:MM."})
                return
            ...
            if digest_type not in ('daily', 'weekly'):
                self.send_json(400, {"status": "error", "message": "Invalid digest type. Must be 'daily' or 'weekly'."})
                return
            ...
            if ref_date:
                try:
                    ...
                    self.send_json(400, {"status": "error", "message": "Invalid reference date format. Must be YYYY-MM-DD."})
                    return
            ...
            try:
                ...
                self.send_json(200, {
                    ...
                    "text": text,
                    "count": result["interviews_count"],
                    "total_upcoming_count": result.get("total_upcoming_count", result["interviews_count"]),
                    "date_range": result["date_range"],
...
def main():
    ...
    parser.add_argument('--host', default='127.0.0.1', help="Host interface for dashboard (default: 127.0.0.1)")
    parser.add_argument('--cron', action='store_true', help="Run in cron mode (generate and dispatch automatically)")
