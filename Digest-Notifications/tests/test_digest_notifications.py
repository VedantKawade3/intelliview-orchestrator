...
from digest import get_upcoming_interviews, generate_digest_html_output, generate_all_outputs, _build_payload, DashboardHandler
...
class TestDigestNotifications(unittest.TestCase):
    ...
    def test_daily_vs_weekly_date_scoping(self):
        ...
        mock_data = [
            {"id": "int-1", "candidate_name": "A", "role": "R", "interviewer_name": "I", "date": day_1, "time": "10:00"},
            {"id": "int-2", "candidate_name": "B", "role": "R", "interviewer_name": "I", "date": day_3, "time": "11:00"},
            {"id": "int-3", "candidate_name": "C", "role": "R", "interviewer_name": "I", "date": day_10, "time": "12:00"}
        ]
        import sqlite3
        ...
        init_conn.execute("CREATE TABLE interviews (id TEXT, candidate_name TEXT, role TEXT, interviewer_name TEXT, date TEXT, time TEXT, status TEXT, meeting_link TEXT, location TEXT)")
        for item in mock_data:
            init_conn.execute("INSERT INTO interviews VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (
                item['id'], item['candidate_name'], item['role'], item['interviewer_name'], item['date'], item['time'], item.get('status', 'Scheduled'), item.get('meeting_link'), item.get('location')
            ))
        init_conn.commit()
    ...
    def test_api_send_invokes_sender_module(self):
        ...
        handler = FakeHandler()
        body_data = json.dumps({"type": "daily", "count": 2, "date_range": "June 27, 2026", "ref_date": "2026-06-27"}).encode('utf-8')
        ...
        db_conn_send_mock.execute("CREATE TABLE sent_logs (id TEXT, timestamp TEXT, type TEXT, count INTEGER, date_range TEXT, recipient TEXT, status TEXT)")
    ...
    def test_concurrent_interview_scheduling(self):
        ...
        import time
        from digest import ThreadingHTTPServer, DashboardHandler
        ...
        try:
            ...
            for row in original_interviews:
                cursor.execute(
                    ...
                    (row['id'], row['candidate_name'], row['role'], row['interviewer_name'], row['date'], row['time'], row['status'], row['meeting_link'], row['location'])
                )
            conn.commit()
            conn.close()
    ...
    def test_digest_truncation_notices(self):
        from models import DigestPayload, DigestRecipient, DigestFrequency, InterviewEvent
        from renderer import render_digest_html, render_digest_text
        import datetime
    ...
    def test_api_pagination(self):
        ...
        interviews_mock = [
            {"id": "1", "candidate_name": "A", "role": "Eng", "interviewer_name": "X", "date": "2026-06-01", "time": "10:00"},
            {"id": "2", "candidate_name": "B", "role": "Eng", "interviewer_name": "Y", "date": "2026-06-02", "time": "11:00"},
            {"id": "3", "candidate_name": "C", "role": "Eng", "interviewer_name": "Z", "date": "2026-06-03", "time": "12:00"}
        ]
        logs_mock = [
            {"id": "l1", "timestamp": "2026-06-01T10:00:00", "type": "Daily", "count": 1, "date_range": "2026-06-01", "recipient": "r1", "status": "Sent"},
            {"id": "l2", "timestamp": "2026-06-02T10:00:00", "type": "Daily", "count": 2, "date_range": "2026-06-02", "recipient": "r2", "status": "Sent"},
            {"id": "l3", "timestamp": "2026-06-03T10:00:00", "type": "Daily", "count": 3, "date_range": "2026-06-03", "recipient": "r3", "status": "Sent"}
        ]
        ...
        db_conn_mock.execute("CREATE TABLE interviews (id TEXT, candidate_name TEXT, role TEXT, interviewer_name TEXT, date TEXT, time TEXT, status TEXT, meeting_link TEXT, location TEXT)")
        for item in interviews_mock:
            db_conn_mock.execute("INSERT INTO interviews VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (
                item['id'], item['candidate_name'], item['role'], item['interviewer_name'], item['date'], item['time'], item.get('status', 'Scheduled'), item.get('meeting_link'), item.get('location')
            ))
        db_conn_mock.commit()
        ...
        db_conn_logs_mock.execute("CREATE TABLE sent_logs (id TEXT, timestamp TEXT, type TEXT, count INTEGER, date_range TEXT, recipient TEXT, status TEXT)")
        for item in logs_mock:
            db_conn_logs_mock.execute("INSERT INTO sent_logs VALUES (?, ?, ?, ?, ?, ?, ?)", (
                item['id'], item['timestamp'], item['type'], item['count'], item['date_range'], item['recipient'], item['status']
            ))
    ...
    def test_scheduled_cron_dispatch(self):
        ...
        init_conn.row_factory = sqlite3.Row
        init_conn.execute("CREATE TABLE interviews (id TEXT, candidate_name TEXT, role TEXT, interviewer_name TEXT, date TEXT, time TEXT, status TEXT, meeting_link TEXT, location TEXT)")
        init_conn.execute("CREATE TABLE sent_logs (id TEXT, timestamp TEXT, type TEXT, count INTEGER, date_range TEXT, recipient TEXT, status TEXT)")
        init_conn.execute("INSERT INTO interviews VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (
            "int-1", "John Doe", "Engineer", "Sarah", "2026-07-15", "10:00", "Scheduled", None, None
        ))
        init_conn.commit()
